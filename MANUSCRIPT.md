...
*v5 draft — restructured around substrate-as-primary-contribution. See `MANUSCRIPT_v4_REWRITE.md` for the prior version.*
...
The substrate's representational scope is loss-of-function and differentiation-collapse biology, which canonical substrate edges can carry; gain-of-function neomorphic biology (KRAS-G12X, EGFR-L858R) is outside scope and is directly falsified on TCGA_LUAD (DUSP4 +2.2 log₂ in KRAS-mut at p = 8×10⁻⁹ in raw RNA; no substrate-routed α-PC clears AUC 0.70). This boundary is a property of the substrate's canonical edges, not of the method, and motivates the planned mutation-conditional edge-injection extension (Paper 4). The substrate (CC-BY 4.0) and every cohort GIZMO has been applied to are deposited on Zenodo as a curated set of per-cohort case studies — F matrix, β/α decomposition, signed-basin output, MOFA+ comparison weights, per-cohort writeup — so each cohort is reusable as a reference projection for downstream methods consuming the substrate.
...
**Scope condition stated up front.** The substrate's representational scope is loss-of-function and differentiation-collapse biology — pathway dampening on existing canonical edges (TP53 truncation collapsing tumor-suppressor reactions; HIF stabilization upregulating canonical hypoxia targets; SLE type-I IFN response engaging the canonical IFN-stimulated gene set). Gain-of-function neomorphic biology — where a mutation creates a *new* edge that doesn't exist in the canonical reference (IDH1-R132H producing 2-hydroxyglutarate as a non-canonical enzymatic product; KRAS-G12X constitutively GTP-bound, engaging effector partners only under mutation; EGFR-L858R structurally rewiring kinase-substrate specificity) — is *outside scope by design*. The canonical substrate cannot represent edges that don't yet exist. This boundary is empirical (TCGA-LUAD KRAS-vs-EGFR subtype discrimination fails by ~AUC 0.45; falsification test in §5) and is the framing motivation for the planned mutation-conditional edge-injection extension (Paper 4). We position this scope condition deliberately rather than apologetically: the framework is built for the biology its substrate can represent, with the boundary stated.
...
**MAP reconstruction.** For each patient i, the state vector F_i over substrate nodes is the MAP solution

  F_i = argmin_F [ ‖x_i − A_obs F‖²_Σ⁻¹ + λ · F^T L_signed F ]

where x_i is the patient's feature vector, A_obs is the substrate-anchoring matrix, Σ is per-modality measurement-variance diagonal, L_signed is the signed substrate Laplacian (catalytic-product edges = +1, inhibitory-substrate edges = −1, neutral = +1; the Dirichlet-smoothness term penalizes sign-discordant transitions across edges), and λ is the smoothing hyperparameter. The solve is closed-form via sparse conjugate-gradient at ~30 sec per patient on the 38,148-node substrate. F output is a per-patient 38,148-dimensional vector in *substrate coordinates*: the i-th entry of F is the inferred state of the i-th substrate node for this patient, regardless of which features the cohort actually measured. This is the key architectural property — patient state is reported in node-coordinates that don't depend on cohort or modality.

**β/α decomposition.** F decomposes uniquely into a hub-projection scalar and an orthogonal residual. The substrate's log-PageRank direction (log_PR_centered, a 38,148-dim unit vector that depends only on substrate topology) defines:

  **β_i** = (F_i · log_PR_centered) / ‖log_PR_centered‖²  — scalar projection onto the hub direction
  **α_i** = F_i − β_i · log_PR_centered  — orthogonal residual (38,148-dim vector)

By construction α_i ⊥ log_PR_centered; β and α decompose F into a hub-aligned component and a substrate-orthogonal component without information loss. The interpretation comes from what log_PR's top-percentile nodes turn out to be empirically. Annotation of the top-50 substrate hub nodes (Supp Table S3) shows: 18 transcriptional / chromatin hubs (TP53, STAT1/3, MITF, NFE2L2, RUNX1, CTNNB1, XBP1, …); 13 signaling / kinase hubs (EGFR, HSP90AA1, SHC1, TYK2, GPCR cascades, …); 8 immune / inflammation reactions (granule exocytosis, IFNG-stimulated gene expression, COP9 signalosome, …); 4 host-defense; 4 viral-cofactor; 1 keratin / structural; 2 mitochondrial-currency. **Forty-seven of the 50 top hub nodes are transcription / signaling / immune / host-defense** — biology that is engaged across virtually all disease states. The β axis therefore reads as a **phenotype-presentation magnitude**: how much of the substrate's most-engaged biology is active in this patient. α is the **orthogonal residual mechanism**: the disease-specific perturbation not captured by phenotype-presentation alone.
...
- **2-hydroxyglutarate (2HG, the canonical IDH oncometabolite)** — ranks **#1 of 6,406 metabolite nodes** in Trautwein NMR. Same recovery in TCGA_IDH RNA-only is absent (the modality doesn't measure it directly), confirming the recovery is a direct-measurement effect, not a graph artifact.
...
This is the falsification: GoF biology fails not from statistical underpowering (the signal is there at p = 10⁻⁹) but from *representational failure* — the canonical substrate has no edge for KRAS-G12X → DUSP4-up because canonical KRAS doesn't have constitutive DUSP4 upregulation as one of its annotated downstream events. **The framework correctly returns "no signal" rather than fabricating one**; the boundary is a property of the substrate's edges, not of the method. This motivates the planned mutation-conditional edge-injection extension (Paper 4) which adds GoF-specific edges conditional on the patient's mutation status; that paper's test set is exactly these 89 single-driver TCGA_LUAD patients.
...
### The LoF/GoF scope boundary is a property of the substrate

Loss-of-function biology and differentiation-collapse biology — pathway dampening on existing canonical edges, hub-stabilization-mediated upregulation of canonical targets, type-I IFN response engagement of the canonical ISG set — live on the substrate by construction. The substrate's edges encode these reactions. Gain-of-function neomorphic biology — KRAS-G12X constitutively active, EGFR-L858R structurally rewired, IDH1-R132H producing 2HG via a non-canonical enzymatic activity — does *not* live on the canonical substrate because the canonical reference doesn't contain those novel edges. The framework cannot represent biology its substrate can't represent; this is not a method limitation, it is a substrate limitation, and it is fixable only by extending the substrate.

The planned **mutation-conditional edge-injection extension (Paper 4)** is the explicit fix. Per-patient mutation calls condition the substrate's edge set — KRAS-G12X patients get an injected DUSP4-up edge that other patients don't see; EGFR-L858R patients get HBEGF-EREG-LRIG1 edges that are absent in canonical EGFR. The 89 single-driver TCGA_LUAD patients (§5 falsification) are the load-bearing test set for whether edge injection moves AUC from the canonical-substrate ceiling (best α-PC AUC 0.64) toward the discrimination floor of ~0.85 observed in cohorts where the substrate already represents the biology. Paper 4 is methodologically constrained by Paper 1's substrate definition; the two papers split the LoF/GoF scope cleanly.

### The substrate is the load-bearing object; the paper series extends it

Paper 1 establishes the canonical substrate and the GIZMO per-patient projection. Paper 2 extends the substrate with explicit lipid flux and state sub-graphs (dual-architecture, compositional Bayes for ionization-bias correction); Paper 3 incorporates disease-informed topology rewiring (network deformation under disease state rather than fixed substrate); Paper 4 adds mutation-conditional GoF edges. Each is an architectural extension of the substrate, exploited by extensions of GIZMO's MAP projection or by other methods consuming the extended substrate. The substrate-as-resource framing is what makes this a paper *series* rather than a sequence of unrelated method papers — each paper grows the shared infrastructure that downstream methods consume.
...
Per-patient state vector solves F = argmin_F [ ‖x − A_obs F‖²_Σ⁻¹ + λ · F^T L_signed F ] via sparse conjugate gradient (scipy.sparse.linalg.cg). Signed Laplacian uses edge-sign annotations (catalytic-product +1, inhibitory-substrate −1, neutral +1). λ chosen per cohort by Methods §"Pre-MAP smoothing diagnostic". Solve time ~30 sec per patient on 38,148-node substrate. Output is per-patient 38,148-dim F vector in substrate coordinates.

### β/α decomposition

β_i = (F_i · log_PR_centered) / ‖log_PR_centered‖² where log_PR_centered is the substrate's log-PageRank vector mean-centered to a unit-norm direction. α_i = F_i − β_i · log_PR_centered. log_PR_centered is a property of substrate topology, not cohort data, and is therefore the same 38,148-dim unit vector across all cohorts. Hub-direction annotation in Supp Table S3 shows 47/50 top hub nodes are transcription / signaling / immune / host-defense.
...
**Effect-size threshold.** Strength = 2 × \|AUC − 0.5\| for binary clinical labels; strength = \|Spearman ρ\| for continuous labels. Pre-registered minimum effect size = 0.40 across all multi-axis discrimination tests.

**Multiplicity correction.** Per-cohort Bonferroni for within-cohort multi-axis × multi-metadata tests (α = 0.05 / n_cohort_tests, where n_cohort_tests = N_PCs × N_metadata_fields_with_≥5_non-null). Global Bonferroni across all panel cohorts at α = 0.05 / N_total_tests. Per-cohort and global thresholds reported for every comparison; `multi_pc_vs_mofa_factors_augmented.tsv` is the audit log.
